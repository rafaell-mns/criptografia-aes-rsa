...
class EnvelopeDigitalApp:
    ...
    # Exibir frame especificado e ocultar outros
    ...
    def criar_frame_abrir_envelope(self):
        ...
        ttk.Label(frame, text="Abrir Envelope Digital", font=("Arial", 16)).pack(pady=20)

        ttk.Label(frame, text="Chave privada do destinatário (.pem):").pack(anchor="w", padx=5, pady=2)
        self.entrada_priv = ttk.Entry(frame, width=50)
        self.entrada_priv.pack(padx=5, fill="x")
        ttk.Button(frame, text="Selecionar Arquivo", command=lambda: self.escolher_arquivo(self.entrada_priv)).pack(pady=2)
    ...
    def executar_criar_envelope(self):
        try:
            ...
            # Verificar se o arquivo de chave pública é realmente uma chave pública
            ...
            # Chamada da função criar_envelope
            criar_envelope(
            ...
            self.mostrar_frame(self.frame_menu) # Voltar ao menu inicial
